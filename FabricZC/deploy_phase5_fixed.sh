...
# Safe execution architecture: Prevents terminal crashes during type re-mapping
# ==============================================================================
set -uo pipefail
...
echo "================================================================================"
echo "[+] STEP 3: Invoking Decoupled Self-Healing Compilation Loop..."
echo "================================================================================"
if [ -f "./run_pure_local_compile.sh" ]; then
    # Execute the build natively without allowing compiler warnings to trip a shell crash
...
if [ -f "./auto_push_readme.sh" ]; then
    # Trigger your automated push macro script to sync Phase 5 upstream
    ./auto_push_readme.sh
fi

rm -f ./deploy_phase5_fixed.sh 2>/dev/null || true
